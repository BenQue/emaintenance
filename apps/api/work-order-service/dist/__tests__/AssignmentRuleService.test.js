...
describe('AssignmentRuleService', () => {
    ...
    beforeEach(() => {
        service = new AssignmentRuleService_1.AssignmentRuleService(mockPrisma);
        jest.clearAllMocks();
    });
    describe('createRule', () => {
        const validRuleData = {
        ...
        beforeEach(() => {
            ...
            mockPrisma.user.findUnique.mockImplementation((query) => {
                if (query.where.id === supervisorId) {
                    return Promise.resolve({ role: database_1.UserRole.SUPERVISOR, isActive: true });
                }
                if (query.where.id === technicianId) {
                    return Promise.resolve({ role: database_1.UserRole.TECHNICIAN, isActive: true });
                }
        ...
        it('should throw error if user is not supervisor', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({ role: database_1.UserRole.EMPLOYEE, isActive: true });
            await expect(service.createRule(validRuleData, 'employee-id')).rejects.toThrow('Access denied: create assignment rules requires supervisor or admin role');
        });
        it('should throw error if assigned technician does not exist', async () => {
            ...
            await expect(service.createRule(validRuleData, supervisorId)).rejects.toThrow('Assigned technician not found or inactive');
        });
        it('should throw error if assigned user is not a technician', async () => {
            mockPrisma.user.findUnique.mockImplementation((query) => {
                ...
                if (query.where.id === technicianId) {
                    return Promise.resolve({ role: database_1.UserRole.EMPLOYEE, isActive: true });
                }
                return Promise.resolve(null);
            });
            await expect(service.createRule(validRuleData, supervisorId)).rejects.toThrow('Assigned user must have technician role');
        });
    });
    ...
    describe('updateRule', () => {
        ...
        beforeEach(() => {
            mockPrisma.user.findUnique.mockResolvedValue({ role: database_1.UserRole.SUPERVISOR, isActive: true });
        });
