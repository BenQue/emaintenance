...
describe('NotificationService', () => {
    ...
    beforeEach(() => {
        service = new NotificationService_1.NotificationService(mockPrisma);
        jest.clearAllMocks();
    });
    describe('createNotification', () => {
        ...
        it('should throw error if target user does not exist', async () => {
            ...
            await expect(service.createNotification(notificationData)).rejects.toThrow('Target user not found or inactive');
        });
    ...
    describe('createWorkOrderAssignmentNotification', () => {
        ...
        it('should create work order assignment notification', async () => {
            const result = await service.createWorkOrderAssignmentNotification(workOrderId, userId, 'Test Work Order');
            expect(mockPrisma.notification.create).toHaveBeenCalledWith({
                data: {
                    userId,
                    type: database_1.NotificationType.WORK_ORDER_ASSIGNED,
            ...
            expect(result.type).toBe(database_1.NotificationType.WORK_ORDER_ASSIGNED);
    ...
    describe('createWorkOrderUpdateNotification', () => {
        ...
        it('should create work order update notification', async () => {
            const result = await service.createWorkOrderUpdateNotification(workOrderId, userId, 'Test Work Order', '已更新状态');
            expect(mockPrisma.notification.create).toHaveBeenCalledWith({
                data: {
                    userId,
                    type: database_1.NotificationType.WORK_ORDER_UPDATED,
            ...
            expect(result.type).toBe(database_1.NotificationType.WORK_ORDER_UPDATED);
        });
    });
    ...
    describe('getUserStats', () => {
        beforeEach(() => {
            ...
            mockPrisma.notification.groupBy.mockResolvedValue([
                { type: database_1.NotificationType.WORK_ORDER_ASSIGNED, _count: { type: 5 } },
                { type: database_1.NotificationType.WORK_ORDER_UPDATED, _count: { type: 3 } },
            ]);
        });
    ...
    describe('getNotificationsForSupervisor', () => {
        ...
        it('should throw error if user is not supervisor', async () => {
            ...
            await expect(service.getNotificationsForSupervisor({}, 'employee-id')).rejects.toThrow('Access denied: Requires supervisor or admin role');
        });
    });
});
