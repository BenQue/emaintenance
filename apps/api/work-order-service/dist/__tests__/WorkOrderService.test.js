...
describe('WorkOrderService', () => {
    ...
    beforeEach(() => {
        ...
        mockRepository = new WorkOrderRepository_1.WorkOrderRepository(mockPrisma);
        workOrderService = new WorkOrderService_1.WorkOrderService(mockPrisma);
        ...
        workOrderService.workOrderRepository = mockRepository;
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('updateWorkOrder', () => {
        it('should update work order successfully', async () => {
            ...
            mockRepository.findById.mockResolvedValue(mockExistingWorkOrder);
            mockRepository.update.mockResolvedValue(mockUpdatedWorkOrder);
    ...
    describe('updateWorkOrderStatus', () => {
        ...
        it('should throw error if work order not found', async () => {
            mockRepository.findById.mockResolvedValue(null);
            await expect(workOrderService.updateWorkOrderStatus('wo-1', { status: 'IN_PROGRESS' }, 'tech-1'))
                .rejects.toThrow('Work order not found');
        });
        it('should throw error if user not authorized to update status', async () => {
            ...
            await expect(workOrderService.updateWorkOrderStatus('wo-1', { status: 'IN_PROGRESS' }, 'other-user'))
        ...
        it('should throw error for invalid status transition', async () => {
            ...
            await expect(workOrderService.updateWorkOrderStatus('wo-1', { status: 'PENDING' }, 'tech-1'))
    ...
    describe('getWorkOrderStatusHistory', () => {
        it('should return status history for work order', async () => {
            ...
            mockPrisma.workOrderStatusHistory.findMany.mockResolvedValue(mockStatusHistory);
    ...
    describe('completeWorkOrder', () => {
        ...
        it('should throw error if work order not found', async () => {
            ...
            mockRepository.findById.mockResolvedValue(null);
            await expect(workOrderService.completeWorkOrder(workOrderId, resolutionData, userId)).rejects.toThrow('Work order not found');
        });
        it('should throw error if user is not assigned to work order', async () => {
            ...
            mockRepository.findById.mockResolvedValue(mockWorkOrder);
            mockPrisma.user.findUnique.mockResolvedValue({ role: 'TECHNICIAN' });
            await expect(workOrderService.completeWorkOrder(workOrderId, resolutionData, userId)).rejects.toThrow('Permission denied: only assigned technician or supervisors can complete work order');
        });
        it('should throw error if work order is already completed', async () => {
            ...
            mockRepository.findById.mockResolvedValue(mockWorkOrder);
            await expect(workOrderService.completeWorkOrder(workOrderId, resolutionData, userId)).rejects.toThrow('Work order is already completed');
        });
        it('should allow supervisor to complete work order', async () => {
            ...
            await expect(workOrderService.completeWorkOrder(workOrderId, resolutionData, userId)).resolves.not.toThrow();
        });
    });
    describe('getAssetMaintenanceHistory', () => {
        it('should get asset maintenance history successfully', async () => {
            ...
            mockPrisma.asset.findUnique.mockResolvedValue(mockAsset);
            mockPrisma.maintenanceHistory.findMany.mockResolvedValue(mockMaintenanceHistory);
    ...
    describe('uploadResolutionPhotos', () => {
        it('should upload resolution photos successfully', async () => {
            ...
            mockPrisma.resolutionPhoto.createMany.mockResolvedValue({});
            mockPrisma.resolutionRecord.findUnique.mockResolvedValue(mockUpdatedResolution);
        ...
        it('should throw error if work order has no resolution record', async () => {
            ...
            mockPrisma.workOrder.findUnique.mockResolvedValue(mockWorkOrder);
            await expect(workOrderService.uploadResolutionPhotos(workOrderId, photoPaths, userId)).rejects.toThrow('Work order does not have a resolution record yet');
        });
    });
});
