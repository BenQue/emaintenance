...
export class CSVGenerator {
  private static readonly DEFAULT_COLUMNS: CSVColumn[] = [
    { key: 'id', label: 'ID' },
   ...
   */
  static generateWorkOrderCSV(workOrders: WorkOrderForCSV[], columns?: string[]): string {
   ...
   */
  private static getSelectedColumns(columnKeys?: string[]): CSVColumn[] {
    if (!columnKeys) {
   ...
   */
  private static generateHeaderRow(columns: CSVColumn[]): string {
   ...
   */
  private static generateDataRows(workOrders: WorkOrderForCSV[], columns: CSVColumn[]): string[] {
    return workOrders.map(workOrder => {
      return columns.map(col => {
        const value = workOrder[col.key as keyof WorkOrderForCSV];
   ...
   */
  static getAvailableColumns(): CSVColumn[] {
    return [...this.DEFAULT_COLUMNS];
  }
}