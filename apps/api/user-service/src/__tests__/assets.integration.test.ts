      ...
      asset: {
        ...
        findUnique: jest.fn().mockImplementation(({ where }: any) => {
          const asset = mockAssets.find((a) => {
            ...
            if (where.assetCode) return a.assetCode === where.assetCode;
            return false;
          });
          return Promise.resolve(asset || null);
        }),
        findMany: jest.fn().mockImplementation(({ where, skip = 0, take = 20 }: any) => {
          let filteredAssets = [...mockAssets];
          
          if (where) {
            if (where.location?.contains) {
              filteredAssets = filteredAssets.filter(a => 
                a.location.toLowerCase().includes(where.location.contains.toLowerCase())
              );
            }
        ...
        update: jest.fn().mockImplementation(({ where, data }: any) => {
          ...
          if (assetIndex === -1) return Promise.resolve(null);
