...
class AssetService {
     ...
     * Bulk create assets
     */
    async bulkCreateAssets(bulkData) {
        ...
        const existingChecks = await Promise.all(assets.map(async (asset, index) => {
        ...
        try {
            ...
            const createDataArray = assets.map(asset => ({
                ...asset,
     ...
     */
    async updateAssetOwnership(id, ownership) {
        // Check if asset exists
        const existingAsset = await this.assetRepository.findById(id);
        if (!existingAsset) {
            throw new Error('Asset not found');
     ...
     * Update asset status
     */
    async updateAssetStatus(id, isActive) {
        const existingAsset = await this.assetRepository.findById(id);
        if (!existingAsset) {
            throw new Error('Asset not found');
        }
    ...
    async getAssetStats() {
        ...
        const [totalResult, activeResult, inactiveResult, allAssetsForLocation,] = await Promise.all([
